...
namespace SpoolSamplerNET
{
    public abstract class rpcapi
    {
        ...
        protected IntPtr Bind(IntPtr IntPtrserver)
        {
            ...
            Console.WriteLine("Binding to " + server + " " + PipeName);
            ...
            if (UseNullSession)
            {
                ...
                if (status != 0)
                {
                    Console.WriteLine("RpcBindingSetAuthInfoEx failed with status 0x" + status.ToString("x"));
        ...
        {
            string server = Marshal.PtrToStringUni(IntPtrserver);
            Console.WriteLine("unbinding " + server);
